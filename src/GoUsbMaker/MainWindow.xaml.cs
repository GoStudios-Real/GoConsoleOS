...
namespace GoConsoleOS.GoUsbMaker;
...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        Logger.Initialize(Path.Combine(_source, "system", "logs"), "info", 10);
        Logger.Info($"GoUSB Maker started, source={_source}");

        VersionText.Text = $"v{new InitConfig().General.Version}";
    ...
    private static string DetectSource()
    {
        ...
        while (check != null)
        {
            if (File.Exists(Path.Combine(check, "GoConsoleOS.exe")) ||
                File.Exists(Path.Combine(check, "GoConsole.exe")))
                return check;
    ...
    private void RefreshDrives()
    {
        ...
        MakerStatus.Text = items.Count == 0
            ? "No installable drives found — plug in a USB drive and press Refresh"
            : $"{items.Count} installable drive{(items.Count == 1 ? "" : "s")} found";
    ...
    private async void InstallClick(object sender, RoutedEventArgs e)
    {
        ...
        if (_selected == null)
        {
            MessageBox.Show(this, "Select a USB drive first.", "GoUSB Maker",
        ...
        Logger.Info($"USB install started: source={_source} target={target}");

        try
        {
            await Task.Run(() => UsbInstaller.RunInstall(_source, target, Report));
